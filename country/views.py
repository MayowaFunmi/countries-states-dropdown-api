...
from .models import Country, City, Person, NigeriaStates, LocalGovernmentArea
...
def country_list(request):
    ...
    return render(request, 'country/list.html', {'countries': all_countries, 'total': total})
...
def person_create_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            myform.save()
            return redirect('country:person_add')
    return render(request, 'country/create_person.html', {'form': form})
...
def person_update_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('country:person_change', pk=pk)
    return render(request, 'country/update_person.html', {'form': form})


# AJAX for load cities in a country
...
def country_and_state(request):
    countries = Country.objects.all()

    #if request.method == 'POST':
        #country_code = request.POST['get_country'] # get country code
        ...
        #return render(request, 'country/country_states.html', {'name_list': name_list, 'country': country, 'total': total})

    return render(request, 'country/get_states.html', {'countries': countries})


def country_and_state_ajax(request):
    data = []
...
# AJAX for load local govts in a state
def load_local_govts(request):
    state_id = request.GET.get('state')
    lgas = LocalGovernmentArea.objects.filter(state_id=state_id).order_by('name')
    return render(request, 'country/lga_dropdown_list_options.html', {'lgas': lgas})
    # return JsonResponse(list(cities.values('id', 'name')), safe=False)


def local_govt_areas(request):
...
def get_states_lga(request):
    states = NigeriaStates.objects.all()
    return render(request, 'country/get_lga.html', {'states': states})


def get_states_lga_ajax(request):
    data = []
    if request.method == 'POST':
        state_name = request.POST['get_state']
        lga_list = []
        for key, state_list in all_lga.items():
            for value in state_list:
                if key == state_name:
                    lga_list.append(value)
        item = {
            'names': lga_list
        }
        data.append(item)
...
class PersonCreateView(CreateView):
    template_name = 'country/create_person.html'
    model = Person
